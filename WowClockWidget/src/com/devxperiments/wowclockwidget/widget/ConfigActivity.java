...
OnPageChangeListener, OnSeekBarChangeListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapter = new ClockAdapter(getSupportFragmentManager());
		...
		imageView.setImageDrawable(WallpaperManager.getInstance(this)
				.getFastDrawable());
		...
		rLayout.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT,
				display.getHeight() / 3));
	...
	private void applyWidget() {
		...
		.putInt(appWidgetId + ClockManager.CLOCK_INDEX_PREF,
				selectedClockIndex)
				.putInt(appWidgetId + ClockManager.HANDS_INDEX_PREF,
						+selectedClock.getCurrentHandsIndex())
						.putInt(appWidgetId + ClockManager.DIAL_INDEX_PREF,
								selectedClock.getCurrentDialIndex())
								.putInt(appWidgetId + ClockManager.DIAL_ALPHA_PREF,
										selectedClock.getDialAlpha())
										.putBoolean(appWidgetId + ClockManager.AM_PM_PREF,
												selectedClock.isAmpm()).commit();

		AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(this);
		appWidgetManager.updateAppWidget(appWidgetId,
				selectedClock.createRemoteViews(this));
	...
	public void onPageSelected(int position) {
		...
		for (int i = 0; i < sizeH; i++)
			handsColorsLayout.addView(buildColorImageView(
					selectedClock.getHands()[i], i, sizeH));


		dialColorsLayout.removeAllViews();
		int sizeD = selectedClock.getDials( ).length;
		for (int i = 0; i < sizeD; i++)
			dialColorsLayout.addView(buildColorImageView(
					selectedClock.getDials()[i], i, sizeD));
